...
def plt_intuition(x_train, y_train):
    ...
    tmp_b = 100 # 假设b固定，w在一定范围内变化

    w_array = np.arange(*w_range, 5) # 分布w的步长为5
    cost = np.zeros_like(w_array) # 创建cost矩阵
    for i in range(len(w_array)): # 对每个w，计算cost
        tmp_w = w_array[i]
        cost[i] = compute_cost(x_train, y_train, tmp_w, tmp_b)

    @interact(w=(*w_range,10),continuous_update=False) # 这个好像是jupyter notebook的交互式控件
    def func( w=150):
        f_wb = np.dot(x_train, w) + tmp_b # 这里用点乘很对劲

        fig, ax = plt.subplots(1, 2, constrained_layout=True, figsize=(8,4)) # 设置两张子图片
        fig.canvas.toolbar_position = 'bottom'

        mk_cost_lines(x_train, y_train, w, tmp_b, ax[0]) # 绘制成本线
        plt_house_x(x_train, y_train, f_wb=f_wb, ax=ax[0]) # 绘制房屋价格线
...
def plt_divergence(p_hist, J_hist, x_train,y_train):
    ...
    cost = np.zeros_like(w_array).astype('int64')
    ...
    z=np.zeros_like(tmp_b).astype('int64')
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j] )
